...
function NavRow({ to, label, icon, end, badge }: { to: string; label: string; icon: Parameters<typeof Icon>[0]['name']; end?: boolean; badge?: string }) {
  return (
...
function NavRowInner({ label, icon, active, badge }: { label: string; icon: Parameters<typeof Icon>[0]['name']; active: boolean; badge?: string }) {
      ...
      <nav className="gx-scroll" style={{ flex: 1, overflowY: 'auto', padding: '12px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {NAV_GROUPS.map((group) => (
          <div key={group.label} style={{ display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 6 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-subtle)', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '10px 12px 6px' }}>
              {group.label}
            </div>
