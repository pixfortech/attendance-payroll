  ...
  return (
    <div style={{ width: 380, maxWidth: '100%' }}>
      ...
      >
        ...
        <Button variant="primary" full type="submit" iconRight={<Icon name="chevronRight" size={16} />}>Sign in to portal</Button>
      </form>
  ...
  if (isMobile) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--surface-page)' }}>
        ...
        <div style={{ background: 'linear-gradient(160deg, var(--indigo-700) 0%, var(--indigo-900) 100%)', color: '#fff', padding: '20px 20px 22px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <img src={gauri} alt="Gauri" style={{ height: 64, filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.3))' }} />
          <div>
            <img src={logo} alt="Ganguram" style={{ height: 30, filter: 'brightness(0) invert(1)' }} />
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', marginTop: 6 }}>Namaste 🙏 Welcome to your employee portal</div>
          </div>
        </div>
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', padding: '28px 20px' }}>
          <LoginForm />
        </div>
      </div>
    );
